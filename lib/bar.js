...
class Bar {
    ...
    addLoad(id, dist, param, is_left, need_bits) {
        // 数字にマッチしなかったら、未指定か、さらにBarがぶら下がっている
        let load;
        if (is_left) {
            load = new Load(`L${id}`, dist, param, need_bits);
            this.leftHand.push(load);
        } else {
            load = new Load(`R${id}`, dist, param, need_bits);
    ...
    constraint(logic, all_bars) {
        ...
        [this.leftHand, this.rightHand].forEach((load_list, i) => {
            load_list.forEach((load) => {
                ...
                if (load.bar !== null) {
                    let cascading_bar = all_bars[load.bar];
                    bits = cascading_bar.getSumBits(all_bars);
                } else {
                    bits = load.bits;
                }
    ...
    getSumBits(all_bars) {
        ...
        this.leftHand.concat(this.rightHand).forEach((load) => {
            if (load.bar !== null) {
                let descent = all_bars[load.bar];
                sum_list.push(descent.getSumBits(all_bars));
            } else {
                sum_list.push(load.bits);
            }
        });
        return Logic.sum(sum_list);
    }
    ...
    toString() {
        ...
        buffer = "  LeftHand: " + this.leftHand.map((load) => {
            if (load.bar !== null) {
                return `${load.name}(d:${load.dist} ->${load.bar})`;
            } else {
