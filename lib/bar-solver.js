...
class BarSolver {
    ...
    localConstraints() {
        ...
        let max_bits = Logic.constantBits(20);
        ...
        this.bar.getBothHands().forEach((load_list, i) => {
            load_list.forEach((load) => {
                ...
                if (load.weight !== null) {
                    ...
                    // 未定の重り
                    bits = Logic.variableBits(load.name, 5);
                    this.solver.require(Logic.greaterThanOrEqual(bits, min_bits));
                    this.solver.require(Logic.lessThanOrEqual(bits, max_bits));
    ...
    solve(bars) {
        ...
        for (let attempt = 1; attempt <= 100; attempt++) {
            ...
            if (load !== null) {
                ...
                for (let value = start_value; value <= 20; value++) {
